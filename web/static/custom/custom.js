	...
	var interesting_subdomain_table = $('#interesting_subdomains').DataTable({
		"drawCallback": function(settings, start, end, max, total, pre) {
			...
			if (this.fnSettings().fnRecordsTotal() == 0) {
				...
				// <div class="card-body">
				// reconPoint could not identify any interesting subdomains. You can customize interesting subdomain keywords <a href="/scanEngine/interesting/lookup/">from here</a> and this section would be automatically updated.
				// </div>
				// </div>`);
			} else {
				// show nav bar
		...
		if (response.status) {
			...
			fetch(`/api/tools/whois/?format=json&ip_domain=${domain_name}&save_db`, {
				...
				if (response.status) {
					...
					Swal.fire({
						title: 'Oops!',
						text: `reconPoint could not fetch WHOIS records for ${domain_name}!`,
						icon: 'error'
					});
				}
			});
...
function get_domain_whois(domain_name, show_add_target_btn=false) {
	...
	fetch(url, {
		...
		if (response.status) {
			...
			Swal.fire({
				title: 'Oops!',
				text: `reconPoint could not fetch WHOIS records for ${domain_name}! ${response['message']}`,
				icon: 'error'
			});
		}
	});
