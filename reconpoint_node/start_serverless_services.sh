...
usage() {
    ...
    -h      Show this message
    Please refer reconPoint command line reference guide for more details.
EOF
}
...
launch_reconpoint() {
    ...
    if [ $auth_result -ne 0 ]; then
        echo "Error: Agent exited. If agent authentication is enabled, check if correct reconpoint key is passed."
        sleep 30
        exit 1
    fi
    if [ "$FEATURE" == "appsec" ]; then
        ...
        envsubst '${SCOPE_HOSTNAME}:${MGMT_CONSOLE_URL}:${MGMT_CONSOLE_PORT}' </home/reconpoint/supervisord-temp.conf >/home/deepfence/supervisord.conf
        unlink /var/run/supervisor.sock 2>/dev/null
        /usr/bin/supervisord -c /home/reconpoint/supervisord.conf
        tail -f /dev/null
    fi
}
...
create_cgroups() {
    ...
    /bin/sh /home/reconpoint/create-cgroups.sh >/dev/null 2>&1
}
...
pidVal=$(/bin/pidof /usr/local/discovery/reconpoint-discovery)
if [ -n "$pidVal" ]; then
